...
#if CC_ALG == LIFE
LifeExecuteResult row_t::execute_life(TxnManager *txn,
                                      const LifeOperation &operation) {
  return manager->execute(txn->life_descriptor(), operation);
}

LifeExecuteResult row_t::execute_life(const LifeTxnDescriptor &descriptor,
                                      const LifeOperation &operation) {
  return manager->execute(descriptor, operation);
}
#endif
