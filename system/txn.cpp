...
void TxnStats::commit_stats(uint64_t thd_id, uint64_t txn_id, uint64_t batch_id,
  ...
  PRINT_LATENCY("lat_l %ld %ld %ld %f %f %f %f %f %f\n", txn_id, batch_id,
                total_work_queue_cnt, (double)timespan_long / BILLION,
...
void TxnManager::life_reset_workload() {}

void TxnManager::life_rollback_descriptor(
    const LifeTxnDescriptor &descriptor, std::vector<Row_life *> &touched_rows) {
  std::vector<Row_life *> rows;
  rows.reserve(touched_rows.size());

  for (std::vector<Row_life *>::const_iterator it = touched_rows.begin();
       it != touched_rows.end(); ++it) {
    if (*it == NULL)
      continue;
  ...
  for (std::vector<Row_life *>::const_iterator it = rows.begin();
       it != rows.end(); ++it) {
    (*it)->rollback(descriptor);
  }

  touched_rows.clear();
}

bool TxnManager::life_finalize_descriptor(
    const LifeTxnDescriptor &descriptor, std::vector<Row_life *> &touched_rows,
  ...
  for (std::vector<Row_life *>::const_iterator it = rows.begin();
       it != rows.end(); ++it) {
    const LifeExecuteResult result = (*it)->prepare(descriptor);
    ...
    if (observed_attempt != NULL && result.code == LifeResultCode::Retry)
      *observed_attempt = result.observed_attempt;

    life_rollback_descriptor(descriptor, touched_rows);
    return false;
  }

  for (std::vector<Row_life *>::const_iterator it = rows.begin();
       it != rows.end(); ++it) {
    (*it)->commit(descriptor);
  }
...
void TxnManager::life_rollback() {
  LifeTxnDescriptor descriptor = life_descriptor();
  life_rollback_descriptor(descriptor, txn->life_objects);
}

void TxnManager::life_reset_attempt(uint64_t observered_attempt) {
  txn->life_tid.attempt =
      std::max(txn->life_tid.attempt, observered_attempt) + 1;

  txn->life_status = LifeTxnStatus::Executing;
  txn->life_history.clear();
  txn->life_objects.clear();
}

bool TxnManager::life_finalize() {
  LifeTxnDescriptor descriptor = life_descriptor();
  uint64_t observed_attempt = txn->life_tid.attempt;

  if (!life_finalize_descriptor(descriptor, txn->life_objects,
                                &observed_attempt)) {
    if (observed_attempt != txn->life_tid.attempt)
      life_reset_attempt(observed_attempt);
    return false;
  }
...
void TxnManager::cleanup(RC rc) {
  ...
  // assert((WORKLOAD == YCSB && row_cnt <= g_req_per_query) || (WORKLOAD ==
  // TPCC && row_cnt <= g_max_items_per_txn*2 + 3));
...
RC TxnManager::get_row(row_t *row, access_t type, row_t *&row_rtn) {
  ...
  if (type == WR) {
    ...
    LogRecord *record = logger.createRecord(get_txn_id(), L_UPDATE,
                                            row->get_table()->get_table_id(),
                                            row->get_primary_key());
    if (g_repl_cnt > 0) {
      msg_queue.enqueue(get_thd_id(), Message::create_message(record, LOG_MSG),
                        g_node_id + g_node_cnt + g_client_node_cnt);
...
bool TxnManager::calvin_collect_phase_done() {
  bool ready = (phase == CALVIN_COLLECT_RD) &&
               (get_rsp_cnt() == calvin_expected_rsp_cnt);
  if (ready) {
