...
uint64_t AbortQueue::enqueue(uint64_t thd_id, uint64_t txn_id,
                             uint64_t abort_cnt) {
  uint64_t starttime = get_sys_clock();
  uint64_t penalty = g_abort_penalty;
#if BACKOFF
  uint64_t calculated_backoff = penalty * pow(2, abort_cnt);
  penalty = min(calculated_backoff, g_abort_penalty_max);
#endif
  penalty += starttime;
  ...
  DEBUG("AQ Enqueue %ld %f -- %f\n", entry->txn_id,
        float(penalty - starttime) / BILLION,
  ...
  while (!queue.empty()) {
    ...
    if (entry->penalty_end < starttime) {
      ...
      DEBUG("AQ Dequeue %ld %f -- %f\n", entry->txn_id,
            ...
            simulation->seconds_from_start(starttime));
      INC_STATS(thd_id, abort_queue_penalty_extra,
                starttime - entry->penalty_end);
